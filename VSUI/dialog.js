...
function dialog(options = {}) {
    ...
    let conHtml = "";
    conHtml = '<div class="' + options.className + '"><div class="weui-mask"></div><div class="weui-dialog">';
    if (options.title) {
        conHtml += '<div class="weui-dialog__hd"><strong class="weui-dialog__title">' + options.title + '</strong></div>';
    }
    conHtml += '<div class="weui-dialog__bd">' + options.content + '</div>';
    conHtml += '<div class="weui-dialog__ft">';
    for (let i = 0; i < options.buttons.length; i++) {
        conHtml += '<a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_' + options.buttons[i]['type'] + '">' + options.buttons[i]['label'] + '</a>';
    }
    conHtml += '</div></div></div>';
